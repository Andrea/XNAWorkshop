...
namespace Behaviours
{
	public class BehavioursLab : Game
	{
		...
		private List<EvadingSprite> _evadingSpritestobeRemoved = new List<EvadingSprite>();
		...
		protected override void Update(GameTime gameTime)
		{
			 ...
			 */
			_lastScardyFishSpawnTime += (int)gameTime.ElapsedGameTime.TotalMilliseconds;
			if (_lastScardyFishSpawnTime -gameTime.ElapsedGameTime.TotalMilliseconds > FishySpawnInterval)
			{
				var initialPosition = new Vector2(-10, GameRandom.Random.Next(_graphics.GraphicsDevice.Viewport.Height));
				_evadingFish.Add(new EvadingSprite(initialPosition, new Vector2(1, 0)));
			}

			foreach (var evadingSprite in _evadingFish)
			{
				// TODO: Remove evadingFish that are not visible anymore

				if (evadingSprite.Position.Y + 100 > _graphics.GraphicsDevice.Viewport.Width)
					_evadingSpritestobeRemoved.Add(evadingSprite);

				evadingSprite.Update(_playerPosition, _playerDirection);
				
			}

			foreach (var evadingSprite in _evadingSpritestobeRemoved)
			{
				_evadingFish.Remove(evadingSprite);
			}
